...
storeMembers :: [Declaration] -> Offset -> Map.Map Identifier Offset -> Map.Map Identifier Offset
storeMembers ((ident, t) : ds) o m
  = storeMembers ds (o + 4) (Map.insert ident o m)
storeMembers [] o m
  = m

storeStruct :: Definition -> (Identifier, Map.Map Identifier Offset)
storeStruct (FunDef (i, _) _)
  = (i, Map.empty)
storeStruct (TypeDef ident members)
  = (ident, storeMembers members 0 Map.empty)

storeStructs :: Program -> [(Identifier, Map.Map Identifier Offset)]
storeStructs p
  = filter (not.null.snd) $ map storeStruct p

generateInstructions :: [(Identifier, Map.Map Identifier Offset)] -> Program -> [Instruction]
generateInstructions sd
  = concatMap $ allocateFuncRegisters . execWriter . flip evalStateT (InstrGenState 0 0 Map.empty sd) . runInstrGen . generateFunction
>>>>>>> codegen: saved struct defs into state
